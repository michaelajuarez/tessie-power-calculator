function calcSessionCost(session, rateplan) {
  ...
  // Manual rate override takes priority over OpenEI structure
  const manualRate = rateplan.manualRate ?? null;

  if (manualRate != null) {
    return { kwh, cost: kwh * manualRate, rateLabel: rateplan.name ?? 'Manual rate' };
  }
  ...
  while (t < endMs) {
    ...
    // Use last tier (above-baseline) since EV charging typically exceeds daily baseline
...
function calcSessionsCost(sessions, rateplan) {
  return sessions.map((s) => ({
    ...s,
    ...calcSessionCost(s, rateplan),
  }));
}

module.exports = { calcSessionCost, calcSessionsCost };
